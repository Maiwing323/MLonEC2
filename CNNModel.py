...
'''
Task 1:
load dataset in MNIST 
n_epochs = 9 ; training 9 example digits
...
def main(argv):
    ...
    # set the parameters
    n_epochs = 9   # set training number of  example digits
    batch_size_train = 64
    ...
    fig = plt.figure()
    for i in range(n_epochs):  # n_epochs = 9
        plt.subplot(3, 3, i + 1)  # 3 * 3 matrix
        plt.tight_layout()
    ...
    model = CNN()  # build a network model
